...
INPT_TYPE block_fixed_arr[12+30];
#endif
#else
...
#ifdef USE_BLOCK_FLOATING_POINT
int leading_zeros(unsigned short x) {
    int n = 0;
    while (x && !(x & 0x8000)) {
        x <<= 1;
        n++;
...
int block_floating_point(INPT_TYPE* data, INPT_TYPE* out_data, int N) {
    INPT_TYPE max_val = 0;
    for (int i = 0; i < N; i++) {
        INPT_TYPE abs_val = (data[i] < 0) ? -data[i] : data[i];
        if (abs_val > max_val) max_val = abs_val;
    }

    int shift = 0;
    if (max_val != 0) {
        shift = leading_zeros(max_val) - 1; // headroom for sign
        for (int i = 0; i < N; i++) {
            out_data[i] = data[i] << shift;  // scale inputs
        }
    }
    return -shift; // return exponent (negative means right-shift later)
}
#endif
...
void fir_filter_fxd_pt(INPT_TYPE* in, COEFF_TYPE* coeffs, INPT_TYPE* out, INPT_TYPE num_of_filt_coeffs, INPT_TYPE frame_size)
{
    ...
    delay = delay_line_fxd;
#ifdef USE_BLOCK_FLOATING_POINT
    //memcpy(block_fixed_arr, delay_line_fxd,(12 + 30) * sizeof(INPT_TYPE));
    shift = block_floating_point(delay_line_fxd,block_fixed_arr,(12+30));
    delay = block_fixed_arr;
#endif
    ...
    for ( n = 0; n < frame_size; n++ ) 
    {
        // calculate out n
        coeffp = coeffs;
        inputp = &delay[num_of_filt_coeffs - 1 + n];
        acc = 0;
        ...
        //acc = acc << (64 - 32 - 3);
        //acc = (((acc >> 46) + 1) >> 1);
        acc = ((acc >> (INTER_PRECISION_BITS-1)) + 1) >> 1;
        // Apply exponent correction (denormalization)
        if (shift < 0)
        {
            acc = acc >> (0 - (shift));      
        }
        out[n] = (INPT_TYPE)(acc); // scale down
#else
        acc = acc << (64 - 32 - 3);
